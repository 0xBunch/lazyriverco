...
type SidebarShellProps = {
  ...
  /// Optional full-viewport-width chrome rendered above the sidebar +
  /// main flex-row. /sports/* passes a <MlsnHeaderBar />. The bar is
  /// expected to expose its own mobile hamburger via useSidebarDrawer();
  /// in turn, the SidebarShell FAB is suppressed on /sports routes so the
  /// two triggers don't double up. When topBar is null/undefined the
  /// existing flat flex-row shape is preserved unchanged.
      ...
      <div className="flex min-h-screen flex-col">
        ...
        <div className="flex flex-1">
          {/* Mobile trigger — only rendered when drawer is closed AND no
              route-specific chrome (like the MLSN red bar) is providing
              its own hamburger. Same env-inset positioning + 44px tap
              target as before. */}
          {!hideMobileFab && !open ? (
            <button
              type="button"
