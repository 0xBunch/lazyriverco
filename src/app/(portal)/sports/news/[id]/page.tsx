    ...
    <main className="w-full">
      ...
      {item.ogImageUrl ? (
        <section className="relative w-full overflow-hidden">
          ...
          <div
            aria-hidden="true"
            className="absolute inset-0 bg-gradient-to-t from-bone-50 via-bone-50/30 to-transparent"
          />
        </section>
      ) : (
        <section
          aria-hidden="true"
          className="relative w-full bg-bone-200"
      ...
      <div className="mx-auto max-w-3xl px-4 py-8 md:px-6 md:py-12">
        ...
        <div className="mb-3 flex flex-wrap items-center gap-2 text-xs">
          <Link
            href="/sports/news"
            className="font-display text-[10px] font-semibold uppercase tracking-[0.28em] text-claude-700 hover:text-claude-800"
          >
            ← Sports news
          </Link>
          ...
          <time
            ...
            className="tabular-nums text-bone-600"
          >
          ...
          {item.sport && (
            <span className="rounded-full bg-bone-100 px-2 py-0.5 text-[10px] tracking-widest text-bone-800 ring-1 ring-bone-300">
              {item.sport}
            </span>
          )}
        </div>

        <h1 className="font-display text-3xl font-semibold tracking-tight text-bone-950 text-balance md:text-4xl lg:text-5xl">
          {item.title}
        </h1>
        {item.author && (
          <p className="mt-3 text-sm text-bone-600">By {item.author}</p>
        )}
            ...
            {item.tags.map((t) => (
              <li key={t}>
                <Link
                  ...
                  className="inline-flex items-center rounded-full bg-bone-100 px-2.5 py-0.5 text-[11px] tracking-widest text-bone-800 ring-1 ring-bone-300 hover:bg-bone-200 hover:text-bone-900"
                >
                  {t}
                </Link>
              </li>
        ...
        {item.excerpt && (
          <p className="mt-6 text-base leading-relaxed text-bone-800 text-pretty md:text-lg">
            {item.excerpt}
          </p>
        )}
        ...
        <div className="mt-8 flex flex-wrap items-center gap-3">
          <a
            ...
            className="inline-flex items-center gap-2 rounded-full bg-claude-600 px-5 py-2.5 text-sm font-medium text-bone-950 hover:bg-claude-500 focus:outline-none focus-visible:ring-2 focus-visible:ring-claude-400"
          >
        ...
        {isAdmin && (
          <section className="mt-10 space-y-6 rounded-2xl border border-bone-200 bg-bone-100 p-5">
            {item.hidden && (
            ...
            <div>
              ...
              <form action={setNewsItemTags} className="mt-3 space-y-3">
                  ...
                  {SPORTS_NEWS_TAGS.map((tag) => {
                    ...
                    return (
                      <label
                        ...
                        className={
                          checked
                            ? "inline-flex cursor-pointer items-center gap-1.5 rounded-full bg-claude-700 px-3 py-1 text-xs text-claude-50 ring-1 ring-claude-700"
                            : "inline-flex cursor-pointer items-center gap-1.5 rounded-full bg-bone-100 px-3 py-1 text-xs text-bone-700 ring-1 ring-bone-200 hover:bg-bone-200 hover:text-bone-900"
                        }
                      >
                        <input
                          type="checkbox"
                ...
                <button
                  type="submit"
                  className="rounded-lg bg-claude-600 px-4 py-2 text-sm font-medium text-bone-950 hover:bg-claude-500"
                >
                  Save tags
                </button>
              </form>
            ...
            <div className="border-t border-bone-200 pt-5">
              ...
              <p className="mt-2 text-xs text-bone-600">
                <strong className="font-semibold text-bone-800">Hide</strong>{" "}
                pulls the story from the public list but keeps the row —
                reversible. <strong className="font-semibold text-bone-800">Delete</strong>{" "}
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <form action={toggleNewsItemHidden}>
                  ...
                  <button
                    type="submit"
                    className="rounded-md border border-bone-300 bg-bone-200 px-3 py-1.5 text-xs font-medium text-bone-900 hover:bg-bone-300"
                  >
                ...
                <form action={deleteNewsItem}>
                  ...
                  <button
                    type="submit"
                    className="rounded-md border border-red-300 bg-red-50 px-3 py-1.5 text-xs font-medium text-red-800 hover:border-red-400 hover:bg-red-100"
                  >
                    Delete permanently
                  </button>
                </form>
              ...
              {related.map((r) => (
                <li key={r.id}>
                  <Link
                    ...
                    className="group flex items-baseline gap-3 rounded-sm py-2 hover:bg-bone-100"
                  >
                    <span className="font-display text-[10px] font-semibold uppercase tracking-[0.28em] text-bone-500">
                      {r.feed.name}
                    </span>
                    <span className="font-display text-sm text-bone-900 group-hover:text-claude-900">
                      {r.title}
                    </span>
                  </Link>
                </li>
