...
// Shared library view. Two routes feed it:
//   /library                — index, filters from query string
//   /library/t/[tag]        — tag page, tag from path + filters from query
// Both prep validated state and call <LibraryView/>; this file owns the
// data loading, the JSX, and the URL-param normalization helpers so the
// two routes stay in lockstep.

export type LibrarySearchParams = {
  q?: string;
  /** Only honored on the index route (legacy /library?tag=X redirect). */
  tag?: string;
  by?: string;
  origin?: string;
...
): LibraryTileItem["origin"] | null {
  return KNOWN_ORIGINS.has(raw ?? "")
    ? (raw as LibraryTileItem["origin"])
    : null;
}
...
export async function LibraryView({
  ...
  const [items, allTags, allMembers] = await Promise.all([
    loadLibraryItems({ q, tag, originFilter, byUserId, limit: GRID_LIMIT }),
    loadDistinctTags(),
    loadMembers(),
  ]);
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 pt-20 md:pt-8">
      <header className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <Link
          href={buildLibraryHref({}, { openAdd: true })}
      ...
      <div className="mb-4 flex gap-2">
        ...
        >
          ...
          />
          {/* Preserve the other filters across search submits. The
              tag rides as ?tag= and gets bounced to /library/t/X by
              the index page's legacy redirect. */}
          {tag ? <input type="hidden" name="tag" value={tag} /> : null}
          {byUserId ? <input type="hidden" name="by" value="me" /> : null}
          {originFilter ? (
